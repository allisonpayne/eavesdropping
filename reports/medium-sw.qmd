...
test_data <- dives %>% filter(dive_id %in% 500:550)
dive_type_plot(test_data, 525, -2:2)

last_dives <- dives %>% filter(dive_id %in% 1000:last(dive_id))

#Plots were generated and saved to outputs folder

#sometimes there is a memory issue 
walk(unique(last_dives$dive_id),
     \(fd) {
       dive_type_plot(dives, fd, -2:2)
       Sys.sleep(0.01)  # Small delay between plots
     })
