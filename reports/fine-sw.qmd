...
Next, load in the acoustic data. We have two kinds of acoustic data: manual annotations of sperm whale bouts, and triton-detected sperm whale clicks. We want to compare them and make sure they look similar.
...
manual <- read.csv(here::here("data/SWCombBouts_Validated.csv")) 
...
ggplot(tpws, aes(clicktime)) + 
  ...
  geom_line(data = sw_manual, aes(date, SW * 700), color = "hotpink") +
  theme_bw()
```
